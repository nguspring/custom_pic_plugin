...
本指南适用于已安装原版插件的用户，通过替换文件的方式升级到修改版 v3.5.1。
...
schedule_mode = "interval"  # interval=倒计时模式，times=指定时间点模式
schedule_times = ["08:00", "20:00"]  # 指定时间点列表
list_mode = "whitelist"  # whitelist=白名单，blacklist=黑名单
chat_id_list = []  # 聊天ID列表
enable_llm_scene = false  # v3.4.2新增：是否启用LLM智能场景判断
scene_llm_model = ""  # v3.4.2新增：智能场景判断使用的模型ID
time_scenes = ["08:00|morning coffee, cafe", "22:00|pajamas, bedroom"]  # v3.4.2新增：Times模式自定义场景
ask_model_id = ""  # v3.4.2新增：询问语生成的模型ID
```
