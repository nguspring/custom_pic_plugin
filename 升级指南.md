...
本指南适用于已安装原版插件的用户，通过替换文件的方式升级到修改版 v3.4.1。
...
schedule_mode = "interval"  # 新增：interval=倒计时模式，times=指定时间点模式
schedule_times = ["08:00", "20:00"]  # 新增：指定时间点列表
list_mode = "whitelist"  # 新增：whitelist=白名单，blacklist=黑名单
chat_id_list = []  # 新增：聊天ID列表
```
