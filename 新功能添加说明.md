...
#### 功能描述
修改版插件将自拍模式的负面提示词拆分为标准自拍和对镜自拍两个独立配置，避免两种模式下的负面提示词冲突。
...
- 自拍负面提示词会与模型默认负面提示词合并
- 避免了两种模式共享一个负面提示词时的冲突问题
...
### plugin.py
- 版本号从 `3.3.3` 更新到 `3.4.0`
- 新增 `auto_selfie` 配置节
- 新增 `search_reference` 配置节
- 新增 `[selfie]` 节下的 `negative_prompt_standard` 和 `negative_prompt_mirror` 配置项
...
### _manifest.json
- 版本号更新至 `3.4.0`
- 新增功能列表：定时自拍、智能参考搜索、自拍模式双负面提示词、内置7个魔搭模型预设
