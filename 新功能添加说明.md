...
| 项目 | 原版插件 (v3.3.3) | 修改版插件 (v3.5.0-beta.6) |
|------|-------------------|---------------------|
| 版本号 | 3.3.3 | 3.5.0-beta.6 |
| 文件数量 | 18个文件 | 34个文件 |
| 新增文件 | 0个 | 16个 |
...
### 0.3 🧠 v3.5.0-beta.2 - Smart智能日程模式（重大更新）
...
### 0.4 🎭 v3.5.0 - 日常叙事系统与配文多样化

#### 功能描述
v3.5.0 版本是定时自拍功能的重大升级，引入了日常叙事系统，让每天的自拍形成连贯的故事线，配文更加自然有承上启下感。同时支持5种配文类型和Hybrid混合模式，使Bot的自拍行为更接近真人习惯。

#### 新增文件
| 文件 | 说明 |
|------|------|
| `core/selfie_models.py` | 基础数据模型（CaptionType、NarrativeScene、DailyNarrativeState） |
| `core/narrative_manager.py` | 日常叙事管理器 |
...
│   ├── selfie_models.py             # v3.5.0 新增：基础数据模型
│   ├── narrative_manager.py         # v3.5.0 新增：日常叙事管理器
...
### selfie_models.py (v3.5.0 新增)
- CaptionType 枚举：定义5种配文类型
- NarrativeScene 数据类：叙事场景信息
- DailyNarrativeState 数据类：每日叙事状态

### narrative_manager.py (v3.5.0 新增)
- 日常叙事管理器
- 内置工作日和周末两套剧本
- 场景匹配和故事线生成
...
### _manifest.json
- 版本号更新至 `3.5.0-beta.5`
- 新增功能列表：定时自拍（多模式）、智能参考搜索、自拍场景配置、内置7个魔搭模型预设、LLM智能场景判断、Times模式自定义场景、日常叙事系统、配文多样化、Hybrid混合模式、Smart智能日程模式、配置结构简化
