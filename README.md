...
基于 Maibot 插件的智能多模型图片生成插件，支持文生图和图生图自动识别。兼容OpenAI、豆包、Gemini、魔搭等多种API格式。提供命令式风格转换、模型配置管理、结果缓存等功能。**v3.5.4 简化配置结构：重写 auto_selfie 配置部分，添加详细中文注释，新增 schedule_times、character_name、character_persona 配置项，删除废弃配置项，让用户配置更加清晰易懂。**
