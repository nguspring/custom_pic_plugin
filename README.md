...
基于 Maibot 插件的智能多模型图片生成插件，支持文生图和图生图自动识别。兼容OpenAI、豆包、Gemini、魔搭等多种API格式。提供命令式风格转换、模型配置管理、结果缓存等功能。**v3.4.2 新增 LLM 智能场景判断功能，支持根据时间自动生成合适的自拍场景；优化询问语生成支持模型选择；修复自拍双手问题。**
...
**功能描述**：开启后，麦麦会按照设定的时间间隔（如60分钟）或指定的时间点（如每天08:00）自动向活跃的聊天群组或私聊发送一张自拍。发送时会附带一句自然的询问语。
...
*   **多时间点模式**：支持每天固定时间点发送（如早安/午安/晚安自拍）。
*   **场景配置化**：可在配置文件中自定义“标准自拍”和“对镜自拍”的场景描述。

**详细配置项** (`[auto_selfie]` 节)：
- `enabled`: 总开关，默认为 false（需手动开启）
- `schedule_mode`: **(新增)** 调度模式。`interval`=倒计时模式，`times`=指定时间点模式
- `schedule_times`: **(新增)** 指定时间点列表，仅在 `times` 模式生效。如 `["08:00", "12:00", "20:00"]`
- `interval_minutes`: 自拍间隔时间（分钟），默认 60 分钟（仅 `interval` 模式生效）
...
- `list_mode`: **(新增)** 名单模式。`whitelist`=白名单（仅允许列表），`blacklist`=黑名单（排除列表）。
- `chat_id_list`: **(新增)** 聊天ID列表。

**自拍场景配置** (`[selfie]` 节 **v3.4.1 新增**)：
