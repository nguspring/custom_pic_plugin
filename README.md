...
基于 Maibot 插件的智能多模型图片生成插件，支持文生图和图生图自动识别。兼容OpenAI、豆包、Gemini、魔搭等多种API格式。提供命令式风格转换、模型配置管理、结果缓存等功能。**v3.5.3 统一使用智能日程模式(Smart Mode)：通过 LLM 动态生成每日日程+场景触发，人物动作完全由场景决定，实现最自然的"真人感"自拍体验。新增间隔补充发送功能，让自拍时间更加随机自然。**
