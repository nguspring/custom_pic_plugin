...
基于 Maibot 插件的智能多模型图片生成插件，支持文生图和图生图自动识别。兼容OpenAI、豆包、Gemini、魔搭等多种API格式。提供命令式风格转换、模型配置管理、结果缓存等功能。**v3.5.1 修复版：修复日常叙事系统死锁问题，配文和VLM默认使用MaiBot内置模型，多群发送性能优化（生成一次，发送多次）。**
...
**v3.5.0 新增特性**：
- 🎭 **日常叙事系统**：每天的自拍形成连贯的故事线（早起→午餐→下班→晚间），配文有承上启下感
- 📝 **配文多样化**：支持5种配文类型（叙事式、询问式、分享式、独白式、无配文），智能选择
- 🔄 **Hybrid 混合模式**：times为主线剧情 + interval为随机惊喜，更接近真人发自拍习惯
- 📅 **剧本系统**：内置工作日和周末两套剧本，自动根据星期切换

**调度模式选择**：
| 模式 | 说明 | 适用场景 |
|------|------|----------|
| `interval` | 每隔N分钟发送一次 | 简单场景 |
| `times` | 每天固定时间点发送 | 规律作息 |
| `hybrid` | times为主线 + interval为补充（推荐） | 拟人效果最佳 |
...
### v3.5.1 (修改版)
- 🐛 **修复死锁问题**：修复日常叙事系统启动时的死锁问题（threading.Lock → RLock）
- 🔧 **配文生成器优化**：`caption_model_id` 留空时默认使用 MaiBot 的 replyer 模型（回复模型）
- 🔧 **智能参考搜索优化**：`vision_api_key` 留空时默认使用 MaiBot 的 vlm 模型（视觉语言模型）
- 📝 **配置说明更新**：更新配置项描述，明确说明默认使用的模型
- 🚀 **性能优化**：定时自拍采用"生成一次，发送多次"模式，多群发送时只调用一次 API 生成图片和配文
- 🔧 **模型选择修复**：修复配文/场景生成时错误选中 embedding 模型的问题，优先使用 replyer/planner/utils
- 🔧 **剧本时间范围优化**：调整日常叙事剧本场景时间范围，消除重叠，与 schedule_times 匹配
...
### v3.4.2 (修改版)
- 🆕 **LLM 智能场景判断**：Interval 模式下可启用 LLM 根据当前时间（时分、星期、日期）自动判断合适的自拍场景（地点、穿着、动作）
- 🆕 **Times 模式自定义场景**：Times 模式支持为每个时间点配置专属场景描述，格式 `"HH:MM|场景描述"`
